<template>
  <div class="overflow-auto"> 
    <v-btn class="m-3" @click.prevent="dialog = true">+ Kas Keluar</v-btn>   
  ...
  export default {
    ...
    methods:{             
        ...
        tambahKas(){
               ...
               this.loading2 = true      
                axios({
                    url: `http://localhost:3000/shift/tambahKas/${this.data._id}`,
                    method: 'put',
                    headers:{
                        ...
                        if (result.isConfirmed) {
                            ...
                            this.loading2 = true
                                const temp = this.kasKeluar.filter(item => String(item._id) !== String(data._id))
                                    axios({
                                        url: `http://localhost:3000/shift/${this.data._id}`,
                                        method: 'put',
                                        headers:{
                                            token : localStorage.getItem('token')
                                        },
